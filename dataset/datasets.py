...
class RawFrameClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        if (mode == 'train'):
            ...
            self.data_transform = video_transforms.Compose([
                ...
                volume_transforms.ClipToTensor(),
                video_transforms.Normalize(
                    mean=[0.1762, 0.1762, 0.1762], std=[0.1826, 0.1826, 0.1826])
            ])
        elif mode == 'test':
            self.data_resize = video_transforms.Compose([
                video_transforms.Resize(
    ...
    def _aug_frame(self, buffer, args):
        ...
        # T H W C
        buffer = tensor_normalize(buffer, [0.1762, 0.1762, 0.1762],
        ...
        buffer = spatial_sampling(
            buffer,
            spatial_idx=-1,
            min_scale=self.short_side_size*0.95,
...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            ...
            frames, _ = video_transforms.uniform_crop(frames, crop_size, 0)
        else:
            transform_func = (
